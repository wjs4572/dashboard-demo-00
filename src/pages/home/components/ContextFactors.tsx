  ...
  return (
    <section className="mb-8">
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <div className="mb-6">
          <h2 className="text-lg font-semibold text-[#1E293B] mb-1">Context and Supporting Factors</h2>
          <p className="text-sm text-gray-600">Additional system context and environmental factors</p>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {factors.map((factor, index) => (
            <div key={index} className="bg-gray-50 rounded-lg p-4">
              <h3 className="text-sm font-semibold text-[#1E293B] mb-4">{factor.category}</h3>
                ...
                {factor.items.map((item, itemIndex) => (
                  <div key={itemIndex} className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="w-2 h-2 rounded-full bg-[#085665]"></div>
                    </div>
                  </div>
                ))}
              </div>
